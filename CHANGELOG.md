...
- Oncolytic virotherapy as a subject, not scattered records: the interferon defect, the approvals and what they really showed, and the virologist who treated herself
- Clicking a value in a table now filters by it: thirteen cells across eight tables, a year facet on trials, and a guard that a filter link can never come back empty
- Click the year to filter the table: thirteen cells across eight tables become filters
- Three owner asks on the roadmap: cells as filters, a record per year, and oncolytic virotherapy
...
- Filtering a table by a cancer family finds its subtypes, the phase filter runs in time order, and the facet search ranks what you typed
- The front page opens on the list again; the graph is one click away
- Four page-render tests were measuring how busy the machine is, not the code
...
- Breast cancer in the NHS: the screening programme the site never had, the family history service, both referral routes, and four wrong or stale funding claims corrected
- One thing, one id: 83 duplicate records merged, with the rule, the helper and the ratchet the three reviews asked for
- Breast cancer, the shared treatment spine: 22 trials on the operation, the axilla, radiotherapy and reconstruction, read at twenty and twenty-five years
- Breast cancer, living with it: the surgery choice with the twenty-year figures, the axilla and lymphoedema, fertility, sensation, hair, and four red cards
...
- Prostate glossary: fifteen new terms, with the screening figures, the two endpoint definitions and PI-RADS against the NICE Likert scale
- Prostate cancer on the NHS: the screening committee's March 2026 recommendation from its own coversheet, the risk in Black men with its cohorts, and the 62-day standard prostate fails worst
- Prostate treatment from its trials: 57 randomised comparisons, what each option costs a man, and the four NICE refusals
- Prostate cancer: five orthogonal classifications settled, the Cambridge Prognostic Groups added, and the decisions written with the trial figures rather than the word side effects
- Register the twelve prostate glossary drafts and link them from the decisions that use them
- Prostate cancer evidence: the 44 papers that made the field, a roadmap in twelve eras, seven ideas and nine open problems
- Prostate cancer, living with it: the ProtecT trade-off in numbers, hormone therapy for the long run, and cord compression named as the emergency
- An agent that has only the origin can now find the corpus: MCP and llms descriptors under .well-known, licence and provenance on every API response
- Prostate cancer has five classifications at once, and only one of them makes pages
- How to read the UK bodies that answer a script with a wall
...
- Target specificity in the pages: pills with glyphs and tooltips on target pages, a "Where it is found" section from the Human Protein Atlas rows, specificity and cancer-type facets on the target browser and the gene hub, and the explainer at /targets/specificity/
- Target specificity: blood-cancer family regex no longer swallows "(all types)" names, family names for the merged blood families, own-record sources for targets the atlases do not cover
- IMPROVEMENTS: rows 150 to 249, the 100 upgrade ideas of 24 September 2026
- Target specificity and distribution: schema, fetcher and data for 904 targets with an approved or clinical-stage drug
- Record pages stay inside a phone screen at 390 px; 130 more trials carry posted registry results
- Gallbladder decision aids and comparison: polyp aid on the 2022 European guideline, incidental cancer aid on AHPBA, Søreide, Ethun and CAPBIL, gallbladder compared with cholangiocarcinoma and ampullary cancer
