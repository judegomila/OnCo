...
- [ ] ctDNA roadmap upkeep: /roadmaps/ctdna-tests/ (eras from 1948 to the 2029 readouts, "What to watch" with registry dates) is refreshed by `npm run ctdna:watch`, which lists every referenced trial with its ClinicalTrials.gov status and completion dates and searches Europe PMC for new papers on the trial acronyms since the roadmap's asOf. Owner: add a weekly cron (GitHub Actions, Mondays with refresh-trials) that runs it and opens an issue with the report; until then run it by hand before each merge round.
