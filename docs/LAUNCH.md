...
- [ ] Owner judgement items from the review: merge duplicate records (cstone/cstone-pharmaceuticals, hengrui/jiangsu-hengrui, pancan/src-pancan, seven reversed-name people ids, three "-term" twins, about 20 journals duplicated as src-* collections); em-dashes in 787 pages of data text (coverage-us access notes, toxicity notes, catalysts, timelines); "as of" in 43 coverage-uk rows and a few TL;DRs; the `spikes/` directory name and `spike` tag; corrections page still links the repo root
