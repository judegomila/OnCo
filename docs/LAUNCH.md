...
- [x] Wave 4 (99 entity pages) and roadmap paging: merged, ship with chain 126 once recall is green **Done:** shipped long before chain 137; the wave 4 pages and the paged roadmaps are live.
...
- [x] www.onco.cc: confirm the domain is attached in Vercel so the redirect in vercel.json applies (curl returned 200 on 10 Sept, not 308) **Done (checked 25 Sept 2026):** `https://www.onco.cc/cancers/prostate/` answers 308 to `https://onco.cc/cancers/prostate/`.
...
- [x] Two bot pull requests are open for your review, opened by the weekly workflows: #1 "Bot proposals: record patches drafted from the feeds" (the human gate for feed-drafted changes; read the diff before merging) and #2 "Weekly refresh: automated pulse, congress abstracts, key-paper citations" (snapshot refresh; low risk). The proposals PR's CI run needed maintainer approval and has been approved so its checks run. **Done:** `gh pr list --state open` is empty; both were applied and closed.
...
- [x] Discovery under `/.well-known/`: `mcp.json` (the server, its tools and the HTTP entry points) and a copy of `llms.txt`, both generated from `MACHINE` by `scripts/build-context.ts` and held in step by `src/lib/agent-surface.test.ts`. The AI crawl policy in `src/app/robots.ts` was already in place. No `ai-plugin.json`: that format requires a contact email and the project does not publish one, so the repository's issues page is the contact in `mcp.json` instead.
- [x] Provenance on API responses: `X-OnCo-License` and `X-OnCo-Provenance` on `/api/v1/*` in `vercel.json`, so an agent that fetches a record without loading the page still gets the licence and the caveat.
...
- [x] Issue 12 (odfalik, 16 Sept): breadcrumb links cannot be clicked because the strip's negative bottom margin lets the page header cover them. Fix: lift the breadcrumb above the header (relative, z-index) and confirm with elementFromPoint on /companies/noetik/. **Done:** issue 12 closed 16 Sept 2026; the breadcrumb nav carries `relative z-10`, which is the fix proposed here.
