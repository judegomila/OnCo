...
- [x] Institution research from OpenAlex: fetcher, Research output panel, /universities/ five-year works and citation columns, weekly workflow. Live with 56 institutions (29 on 11 Sept, 27 more on 12 Sept); OpenAlex meters the free API at 1,000 credits a day and the fetcher is incremental, so `npm run fetch:research` once a day after the UTC reset adds the next batch, or one run finishes it if the owner adds an OPENALEX_API_KEY repo secret
