...
Removed on 21 Sept with the move of accounts to me.onco.cc; kept as history. The Supabase variables named here are no longer read by this site.
...
**Accounts through WorkOS (owner ask, 17 Sept; removed 21 Sept, see above):** the site now signs users in through WorkOS AuthKit with the browser-only PKCE flow (no server, no library; src/lib/account.ts). To switch it on: in the WorkOS dashboard create or pick the environment, under Redirects add `https://onco.cc/signup/` (and `http://localhost:3000/signup/` for local runs) as a redirect URI, under Authentication enable AuthKit and the sign-in methods wanted (email code, password, Google, passkey), and allow the origin `https://onco.cc` for client-side (CORS) requests; then in Vercel set `NEXT_PUBLIC_WORKOS_CLIENT_ID` to the client id of an environment in your personal WorkOS account (not the Ultrametrics workspace; the id starts `client_`) for Production and redeploy. Users appear under User Management in WorkOS, where they can be exported. The Supabase path stays as a fallback when only its keys are set.
...
**Cloud profiles, stage one (21 Sept, removed the same day):** the signed-in profile (role, cancer, country, view, language, theme, consent time) and the kind and id of each watched page are now stored in two Supabase tables under the WorkOS access token (src/lib/cloud-profile.ts, src/lib/cloud-sync.ts, supabase/migrations/0001_profiles.sql). Owner steps, in order, are in supabase/README.md: run the migration, add WorkOS as a third-party auth provider (issuer `https://api.workos.com/user_management/<client_id>`, JWKS `https://api.workos.com/sso/jwks/<client_id>`), add the `"role": "authenticated"` claim to the WorkOS JWT template, then set `NEXT_PUBLIC_SUPABASE_URL` and `NEXT_PUBLIC_SUPABASE_ANON_KEY` in Vercel and redeploy. Until both variables exist the cloud code is a no-op and the account menu shows no sync line.
