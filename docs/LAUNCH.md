...
- [x] Institution research from OpenAlex: fetcher, Research output panel, /universities/ five-year works and citation columns, weekly workflow. Live with 79 institutions (29 on 11 Sept, 27 on 12 Sept, 23 on 13 Sept); OpenAlex now stops the free key after roughly 100 credits a day (the fetcher logs the allowance), so one `npm run fetch:research` a day after the UTC reset adds about 20 institutions, or one run finishes the remaining 472 if the owner adds an OPENALEX_API_KEY repo secret
...
- Institution research: 79 institutions have OpenAlex snapshots. The free key now stops after roughly 100 credits a day, so one `npm run fetch:research` per day after the UTC reset adds about 20; an OPENALEX_API_KEY secret would finish the remaining 472 in one run.
