...
describe("api layout", () => {
  ...
  it("describes every file in the layout as an OpenAPI 3.1 path", () => {
    ...
    for (const f of apiFiles(counts)) {
      const templated = isKindFile(f.path) ? `/api/v1/{plural}.${f.path.endsWith(".csv") ? "csv" : "json"}` : f.path.replace("<id>", "{id}").replace("<plural>", "{plural}").replace("<view>", "{view}").replace("<table>", "{table}").replace("<format>", "{format}");
