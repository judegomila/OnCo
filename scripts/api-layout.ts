...
import { FORMAT_IDS } from "../src/lib/modular-formats";
...
export function apiFiles(counts: Record<Kind, number>): ApiFile[] {
  return [
    ...
    { path: "/api/v1/tables/<table>.json", contents: "Every row of one paged table (kind-<route> for the kind browsers such as kind-trials and kind-key-papers, evidence, china-trials, university-output, university-output-grouped, university-score, audit-*, pathway-matrix, pathway-nodes, dossier-trials-<target>, engine-<format>), in the table's default order; the page carries the first 30 rows (60 for a kind browser) and fetches the rest" },
    { path: "/api/v1/pipeline/engine/<format>.json", contents: "The open drug engine for one format (adc, radioligand, car-t, tcr-t, bispecific, degrader, small-molecule, antibody, cytokine, vaccine, oncolytic-virus, cell-therapy): every medicine taken apart into its components with the fields each was read from and a confidence, the grid cells of the two axis components with their state (approved, development, stopped, unclear) and the records behind it, component counts, the medicines that could not be placed, and every recorded reason for stopping" },
    { path: "/api/v1/pipeline/engine/index.json", contents: "The engine's formats with their axes, cell counts per state (including untried) and decomposer coverage, and the drug records outside every format by reason" },
    { path: "/api/v1/for-me/<id>.related.json", contents: "Everything in OnCo that touches one cancer for the For me picker: state of the art, red cards, pipeline, and the hopeful records grouped by kind" },
...
export function openApiDocument(counts: Record<Kind, number>, opts: { version?: string | null; built?: string } = {}): Json {
  ...
  const paths: Json = {
    ...
    "/api/v1/pipeline/engine/{format}.json": get("getEngineFormat", "The open drug engine for one format: format (id, name, axes, components), counts (cells per state including untried), coverage (medicines placed, partial, unresolved), rows and cols (the axis components), cells (a, b, state, drugs with their own state, counts, the other parts seen, evidence records with record id, field, kind, label, href and any quoted reason, and reasons), components (every value with its medicine count and states), drugs (each medicine with its components, the fields each was read from and a confidence, its state, evidence and reasons), unresolved (medicines the decomposer could not place, with what is missing), stopped (every recorded reason for stopping, in the record's words). States are derived from the named records only; untried means no medicine in the corpus combines the two parts", { ...ok({ type: "object", required: ["format", "counts", "coverage", "rows", "cols", "cells", "components", "drugs", "unresolved", "stopped"], properties: { format: { type: "object", additionalProperties: true }, counts: { type: "object", additionalProperties: { type: "integer" } }, coverage: { type: "object", additionalProperties: { type: "integer" } }, rows: { type: "array", items: { type: "object", additionalProperties: true } }, cols: { type: "array", items: { type: "object", additionalProperties: true } }, cells: { type: "array", items: { type: "object", required: ["a", "b", "state", "drugs", "evidence"], properties: { a: { type: "object", additionalProperties: true }, b: { type: "object", additionalProperties: true }, state: { type: "string", enum: ["approved", "development", "stopped", "unclear"] }, drugs: { type: "array", items: { type: "object", additionalProperties: true } }, evidence: { type: "array", items: { type: "object", required: ["record", "kind", "state", "label", "href"], properties: { record: { type: "string" }, field: { type: "string" }, kind: { type: "string", enum: ["drug", "trial", "approval", "regional", "event", "registry"] }, state: { type: "string" }, label: { type: "string" }, href: { type: "string" }, quote: { type: "string" } } } } }, additionalProperties: true } }, components: { type: "object", additionalProperties: true }, drugs: { type: "array", items: { type: "object", additionalProperties: true } }, unresolved: { type: "array", items: { type: "object", additionalProperties: true } }, stopped: { type: "array", items: { type: "object", additionalProperties: true } } } }), "404": { description: "No such format." } }, { parameters: [{ name: "format", in: "path", required: true, description: "The format id, as in the page URL /pipeline/engine/<format>/.", schema: { type: "string", enum: [...FORMAT_IDS] } }] }),
    "/api/v1/pipeline/engine/index.json": get("getEngineIndex", "The open drug engine's formats: id, name, page route, file, axes, cell counts per state (approved, development, stopped, unclear, untried) with medicine, partial and unresolved counts, and decomposer coverage; plus the drug records outside every format, by reason", ok({ type: "object", required: ["formats", "drugs"], properties: { drugs: { type: "integer" }, fetched: { type: "string" }, formats: { type: "array", items: { type: "object", additionalProperties: true } }, outside: { type: "array", items: { type: "object", required: ["reason", "count"], properties: { reason: { type: "string" }, count: { type: "integer" } } } } } })),
    "/api/v1/for-me/{id}.related.json": get("getForMeRelated", "Everything in OnCo that touches one cancer, for the For me picker: state of the art, red cards, the pipeline, and the hopeful records (failures, withdrawals and historic items left out) grouped by kind as id, name, TL;DR, route and status", { ...ok({ type: "object", additionalProperties: true }), "404": { description: "No such cancer id." } }, { parameters: [idParam("Kebab-case cancer id, for example tnbc or nsclc.")] }),
