...
export function apiFiles(counts: Record<Kind, number>): ApiFile[] {
  return [
    ...
    { path: "/api/v1/tables/<table>.json", contents: "Every row of one paged table (evidence, china-trials, university-output, university-output-grouped, university-score, audit-*, pathway-matrix, pathway-nodes, dossier-trials-<target>), in the table's default order; the page carries the first 30 rows and fetches the rest" },
    ...
    { path: "/api/v1/explained/<id>.json", contents: "The outcomes, setting and enrolment of every trial with structured results for one cancer (or 'other' for trials not yet linked to a cancer), keyed by trial id: what Trials in plain words explains on demand" },
...
export function openApiDocument(counts: Record<Kind, number>, opts: { version?: string | null; built?: string } = {}): Json {
  ...
  const paths: Json = {
    ...
    "/api/v1/explained/{id}.json": get("getExplainedSection", "For one cancer (or 'other'), every trial with structured results as id, outcomes, setting and enrolment, keyed by trial id: the records the Trials in plain words page turns into sentences when a heading is opened", { ...ok({ type: "object", required: ["trials"], properties: { trials: { type: "object", additionalProperties: { type: "object", additionalProperties: true } } } }), "404": { description: "No such cancer id, or no trials with results for it." } }, { parameters: [idParam("Kebab-case cancer id, for example tnbc or nsclc, or 'other'.")] }),
