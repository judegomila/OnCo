...
export function apiFiles(counts: Record<Kind, number>): ApiFile[] {
  return [
    ...
    { path: "/api/v1/tables/<table>.json", contents: "Every row of one paged table (kind-<route> for the kind browsers such as kind-trials and kind-key-papers, tag-<slug> for the tag pages, evidence, china-trials, university-output, university-output-grouped, university-score, audit-*, pathway-matrix, pathway-nodes, dossier-trials-<target>), in the table's default order; the page carries the first 30 rows (60 for a kind or tag browser) and fetches the rest" },
    { path: "/api/v1/tagged/<id>.json", contents: "One tag (by its slug): its spellings, description, counts per kind, related tags and every record carrying it as id, kind, name, route, status and TL;DR; the companion of /tagged/<slug>/" },
    { path: "/api/v1/tagged/index.json", contents: "Every public tag as slug, canonical spelling, description, count and counts per kind" },
...
export function openApiDocument(counts: Record<Kind, number>, opts: { version?: string | null; built?: string } = {}): Json {
  ...
  const paths: Json = {
    ...
    "/api/v1/tagged/{id}.json": get("getTag", "One tag by its slug: slug, tag (canonical spelling), variants (every spelling sharing the slug), description, count, kinds (records per kind), related (tags sharing records, with the shared count) and records, every record carrying it as id, kind, name, route, status and tldr; the companion of /tagged/{slug}/", { ...ok({ type: "object", required: ["slug", "tag", "variants", "description", "count", "kinds", "related", "records"], properties: { slug: { type: "string" }, tag: { type: "string" }, variants: { type: "array", items: { type: "string" } }, description: { type: "string" }, count: { type: "integer" }, kinds: { type: "object", additionalProperties: { type: "integer" } }, related: { type: "array", items: { type: "object", required: ["slug", "tag", "shared"], properties: { slug: { type: "string" }, tag: { type: "string" }, shared: { type: "integer" } } } }, records: { type: "array", items: { type: "object", required: ["id", "kind", "name", "route"], properties: { id: { type: "string" }, kind: ref("Kind"), name: { type: "string" }, route: { type: "string" }, status: ref("Status"), tldr: { type: "string" } } } } } }), "404": { description: "No record carries a tag with this slug." } }, { parameters: [idParam("The tag slug: lower case, hyphens between words, as in the /tagged/ URLs.")] }),
    "/api/v1/tagged/index.json": get("listTags", "Every public tag as slug, tag (canonical spelling), variants, description, count and kinds (records per kind), most used first", ok({ type: "array", items: { type: "object", required: ["slug", "tag", "count", "kinds"], properties: { slug: { type: "string" }, tag: { type: "string" }, variants: { type: "array", items: { type: "string" } }, description: { type: "string" }, count: { type: "integer" }, kinds: { type: "object", additionalProperties: { type: "integer" } } } } })),
