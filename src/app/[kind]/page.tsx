...
export default async function KindIndex({ params }: { params: Promise<{ kind: string }> }) {
  ...
  const right = k === "cancer" ? <div className="flex flex-wrap gap-2 justify-end"><Link href="/cancers/map/" title="Every cancer type on one layered map: organ system, cancer, subtype" className="rounded-lg border border-border bg-card px-4 py-2 text-sm font-medium">Whole map →</Link><Link href="/for-me/" className="rounded-lg bg-accent text-white px-4 py-2 text-sm font-medium">Pick mine →</Link></div>
    ...
    : k === "target" ? <div className="flex flex-wrap gap-2 justify-end"><Link href="/targets/specificity/" title="Is a target unique to cancer cells, shared with normal tissue or a lineage, everywhere, inherited, or on immune cells?" className="rounded-lg border border-border bg-card px-4 py-2 text-sm font-medium">Specificity classes →</Link><Link href="/targets/genome/" title="Every gene and protein the open catalogues tie to cancer, grouped by role and evidence" className="rounded-lg border border-border bg-card px-4 py-2 text-sm font-medium">Cancer genes by role →</Link></div>
