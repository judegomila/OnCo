    ...
    <>
      ...
      <Container className="pb-16 max-w-3xl">
        <div className="card p-4 mb-6 text-sm border-accent/30 bg-accent-soft/40">
          ...
          <p>The data are copyright OnCo and free for individual and educational use, including academic and non-profit use, under <a className="underline" href="https://creativecommons.org/licenses/by-nc/4.0/" rel="noopener">CC BY-NC 4.0</a>, with the attribution &ldquo;Data from OnCo (onco.cc)&rdquo;: name OnCo and link to <a className="underline" href="https://onco.cc">onco.cc</a> wherever the data or text derived from it appears, for example <code className="text-xs">Data from OnCo (onco.cc), CC BY-NC 4.0</code>. Commercial use (inside a paid product or service, advertising-funded redistribution, or a company data pipeline) must <Link className="underline" href="/about/#licence">contact us to pay for the data</Link>; the same terms apply to the API, the files and the CLI. Logos remain their owners&apos; trademarks and molecule structures keep their PubChem and RCSB terms. The same notice is in <code className="text-xs">/api/v1/meta.json</code>, as the first line of every CSV and inside every JSON download from the site&apos;s tables.</p>
        </div>
        ...
        <table className="onco">
          ...
          <tbody>
            ...
            <tr><td><a className={mono} href="/api/v1/my-cancers.json">/api/v1/my-cancers.json</a></td><td>Every cancer as id, name, route and hub group: the chooser list the site fetches on demand.</td></tr>
            <tr><td><a className={mono} href="/api/v1/spotlight.json">/api/v1/spotlight.json</a></td><td>The home page spotlight: one hero record per kind in the daily rotation, with the selection rule and the schedule.</td></tr>
