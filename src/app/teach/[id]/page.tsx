...
function cancerSlides(c: Cancer): Slide[] {
  ...
  const slides: Slide[] = [
    ...
    { id: "what", kicker: "What it is", title: "Summary", paragraphs: cut(summary, 2), notes: cut(summary.slice(2), 2), sources },
    { id: "soc", kicker: "Standard of care", title: "Standard of care by setting", table: c.standardOfCare.length ? { head: ["Setting", "Approach", "Guideline"], rows: cut(c.standardOfCare, 8).map((s) => [s.setting, s.approach, [s.guideline?.nccn ? `NCCN ${s.guideline.nccn}` : "", s.guideline?.esmoMcbs ? `ESMO-MCBS ${s.guideline.esmoMcbs}` : ""].filter(Boolean).join(", ") || "not mapped"]) } : undefined, notes: cut(c.standardOfCare, 8).map((s) => `${s.setting}: evidence from ${s.refs.length ? s.refs.map(nameOf).join(", ") : "guideline consensus"}${s.guideline?.version ? ` (${s.guideline.version})` : ""}.`), sources: guidelineUrls },
    { id: "art", kicker: "State of the art", title: "Current frontier", bullets: cut(c.stateOfArt, 6), notes: c.stateOfArt.slice(6, 9) },
    { id: "history", kicker: "History", title: "Milestones", timeline: cut([...c.history].sort((a, b) => String(a.year).localeCompare(String(b.year))), 10).map((h) => ({ year: String(h.year), title: h.title })), notes: cut(c.history.filter((h) => h.note), 6).map((h) => `${h.year}: ${h.note}`) },
    { id: "pipeline", kicker: "Pipeline", title: "Coming next", bullets: cut(c.pipeline, 12).map((id) => `${nameOf(id)} (${kindOf(id)})`), notes: cut(c.pipeline, 6).map((id) => `${nameOf(id)}: ${g.get(id)?.tldr ?? ""}`) },
    { id: "trials", kicker: "Evidence", title: "Standard-setting trials", bullets: trials.map(trialLine), notes: trials.map((t) => `${t.name}${t.nct ? ` (${t.nct})` : ""}: ${t.result ?? "no headline result recorded"}${t.replication ? ` Replication: ${t.replication}` : ""}`), sources: trials.flatMap((t) => t.links.slice(0, 1)) },
    { id: "problems", kicker: "Open problems", title: "Unsolved problems", bullets: cut(c.openProblems, 6), notes: c.openProblems.slice(6, 9) },
    { id: "quiz", kicker: "Quiz", title: "Quiz questions", quiz: quizFor(c.id, related), notes: ["Questions come from OnCo's open benchmark (src/data/benchmark.ts); the model answers reflect the corpus on the date each record was last checked."] },
    { id: "sources", kicker: "Sources", title: "Primary sources", bullets: cut([...sources, ...guidelineUrls], 12).map((s) => `${s.label}: ${s.url}`), notes: [`Record last updated ${c.asOf}. Corrections: ${absoluteUrl("/suggest/")}.`] },
  ];
...
function frontSlides(s: Section): Slide[] {
  ...
  const slides: Slide[] = [
    ...
    { id: "what", kicker: "What it is", title: "Summary", paragraphs: cut(summary, 2), notes: cut(summary.slice(2), 2), sources },
    { id: "techs", kicker: "Technologies", title: "Technologies on this front", bullets: cut(techs, 10).map((t) => `${t.name}: ${t.tldr}`), notes: cut(techs, 6).map((t) => `${t.name}: ${t.principle}`) },
    { id: "products", kicker: "Products", title: "Approved products", bullets: cut([...drugs].sort((a, b) => (a.status === "approved" || a.status === "standard-of-care" ? -1 : 1) - (b.status === "approved" || b.status === "standard-of-care" ? -1 : 1)), 10).map((d) => `${d.name}${d.status ? ` (${d.status})` : ""}: ${d.tldr}`), notes: cut(drugs, 6).map((d) => `${d.name}: ${d.mechanism}`) },
    ...
    { id: "trials", kicker: "Evidence", title: "Trials that moved the front", bullets: trials.map(trialLine), notes: trials.map((t) => `${t.name}${t.nct ? ` (${t.nct})` : ""}: ${t.result ?? "no headline result recorded"}`), sources: trials.flatMap((t) => t.links.slice(0, 1)) },
    { id: "quiz", kicker: "Quiz", title: "Quiz questions", quiz: quizFor(s.id, related), notes: ["Questions come from OnCo's open benchmark (src/data/benchmark.ts)."] },
    { id: "sources", kicker: "Sources", title: "Primary sources", bullets: cut(sources, 12).map((x) => `${x.label}: ${x.url}`), notes: [`Record last updated ${s.asOf}. Corrections: ${absoluteUrl("/suggest/")}.`] },
  ];
