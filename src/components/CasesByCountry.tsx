...
export function CasesByCountry({ cancers, initial, countryList, dataUrl, gaps, meta }: CasesProps) {
  ...
  const columns: Column<CountryRow>[] = [
    ...
    { key: "region", label: "Region", sortable: true, hide: "hidden md:table-cell", filter: { options: regionOptions, value: regions, onChange: setRegions }, render: (r) => <span className="text-muted">{r.region}</span> },
    { key: "hdi", label: "HDI", hide: "hidden lg:table-cell", filter: { options: hdiOptions, value: hdi, onChange: setHdi }, render: (r) => <span className="text-muted">{r.hdi?.replace(" HDI", "") ?? "-"}</span> },
