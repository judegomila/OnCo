...
export function GitHubStars({ className = "" }: { className?: string }) {
  ...
  return (
    <a href={`https://github.com/${REPO}`} rel="noopener" title={t("header.starsTitle", { n: stars.toLocaleString("en-GB") })} aria-label={t("header.starsTitle", { n: stars.toLocaleString("en-GB") })}
      ...
      {/* Between xl and 2xl the count folds into the icon (the title and aria-label keep it): with a classic scrollbar a 1280 px window is 1263 px wide, and the full header needed 1326 px (23 Sept 2026). */}
      <span className="ctl-label inline-block min-w-[3ch] text-start xl:hidden 2xl:inline-block">{label}</span>
    </a>
  );
}
