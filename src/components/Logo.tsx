 ...
 */
export function Logo({ id, website, name, size = 56, className = "", rounded = "rounded-xl" }: { id?: string; website?: string; name: string; size?: number; className?: string; /** Corner radius class; small tiles inside pills want a tighter one. */ rounded?: string }) {
  ...
  return (
    <span className={`relative inline-flex shrink-0 items-center justify-center ${rounded} border border-border bg-white overflow-hidden ${className}`} style={{ width: size, height: size }} title={src ? title : undefined} aria-hidden>
      <span className={`absolute inset-0 flex items-center justify-center font-semibold text-zinc-500 ${size < 28 ? "text-[8px]" : "text-sm"}`}>{initials}</span>
      {src && (
