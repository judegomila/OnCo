...
export function PipelineFunnel({ funnels, crowding, fetched }: { funnels: Funnel[]; crowding: CrowdingRow[]; fetched: string }) {
  ...
  const population = (r: CrowdingRow) => (r.index === null ? "No estimate" : r.addressable?.dropped.length ? "Partial population" : "Complete population");
  const approvedOf = (r: CrowdingRow) => (r.approved ? "Has an approved product" : "None approved");
  ...
  const rows = useMemo(() => {
    const list = crowding.filter((r) => r.active >= minActive && hf.pass("population", [population(r)]) && hf.pass("approved", [approvedOf(r)]));
  ...
  }, [crowding, sort, minActive, hf.sel]);
  ...
  const columns: Column<CrowdingRow>[] = [
    ...
    { key: "approved", label: "Approved", sortable: true, filter: hf.spec("approved", countOptions(crowding.filter((r) => r.active >= minActive).map(approvedOf), { order: ["Has an approved product", "None approved"] })), render: (r) => <span className="tabular-nums">{r.approved || <span className="text-muted">0</span>}</span> },
    ...
    <div className="space-y-10">
      ...
      <section>
        <div className="flex flex-wrap items-baseline justify-between gap-3 mb-3">
          ...
          {hf.active && <button type="button" onClick={hf.clear} className="text-sm underline text-muted">Clear header filters</button>}
