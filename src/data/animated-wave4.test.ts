...
describe("wave 4 animated technology schematics", () => {
  const keys = Object.keys(WAVE4);

  it("covers forty technologies, every key a real technology id, all registered in ANIMATED", () => {
  ...
  it("every scene stays under 500 points, keeps finite coordinates and matching point/alpha counts, and captions four phases", { timeout: 60_000 }, () => {
    for (const [key, build] of Object.entries(WAVE4)) {
      const m = build();
      ...
      for (const c of captions) { expect(c, `${key} caption numbered`).toMatch(/^[1-4] · /); expect(c, `${key} caption without em-dash`).not.toContain("—"); }
    }
  });
