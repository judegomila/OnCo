...
describe("wave 5 animated technology schematics", () => {
  const keys = Object.keys(WAVE5);

  it("covers forty technologies, every key a real technology id not already in waves 3 or 4, all registered in ANIMATED", () => {
    expect(keys.length).toBe(40);
    const g = graph();
    for (const k of keys) {
      const e = g.get(k);
      expect(e?.kind, `${k} is a technology`).toBe("technology");
      expect(k in WAVE3 || k in WAVE4, `${k} not in an earlier wave`).toBe(false);
