...
describe("wave 7 animated technology schematics", () => {
  const keys = Object.keys(WAVE7);

  it("covers forty technologies, every key a real technology id not already in waves 3 to 6, all registered in ANIMATED", () => {
    expect(keys.length).toBe(40);
    const g = graph();
    for (const k of keys) {
      const e = g.get(k);
      expect(e?.kind, `${k} is a technology`).toBe("technology");
      expect(k in WAVE3 || k in WAVE4 || k in WAVE5 || k in WAVE6, `${k} not in an earlier wave`).toBe(false);
  ...
  it("every scene stays under 500 points, keeps finite coordinates and matching point/alpha counts, and captions four phases", { timeout: 60_000 }, () => {
    for (const [key, build] of Object.entries(WAVE7)) {
      const m = build();
