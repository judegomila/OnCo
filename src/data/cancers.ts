...
export const cancers: CancerInput[] = [
  ...
  c({
    ...
    // The four one-line settings that stood here (Early stage; Metastatic first line; Metastatic second line;
    // Endocrine-resistant) are each written at length, with their trials and their figures, by the eleven rows in
    // src/data/spikes/breast-hr-positive.ts. "Metastatic first line" therefore rendered twice on the page, once as
    // a sentence and once as a paragraph. Every id they referenced is still reachable from the rows that survive;
    // the NCCN guideline summary they cited is now a link on this record so it remains a source of the record.
    // Removed by the September 2026 breast family review (docs/BREAST-QA.md), following the lung review's rule 24.
    // The three one-line state-of-the-art lines that stood here are restated, with their trials, by the same file.
    history: [
      // The six milestones that stood here (1896 Beatson, 1977 tamoxifen, 2015 palbociclib, 2018 TAILORx, 2022
      // HER2-low, 2026 PROTAC) are each written again, with their notes and trials, by
      // src/data/spikes/breast-hr-positive.ts; 1977 and 2018 rendered as two identical rows under Milestones.
      // `endocrine-therapy` stays reachable from this record's technologies and `oncotype-dx` from the spike's
      // chemotherapy-decision row.
    ],
    ...
    // The three one-line open problems that stood here (late recurrence with no predictive test; CDK4/6 resistance
    // and sequencing; endocrine adherence and toxicity) are each written with their figures by
    // src/data/spikes/breast-hr-positive.ts, which is where they now appear once.
  ...
  c({
    ...
    // The two one-line settings that stood here (Early stage; Metastatic) are written at length, with their trials
    // and their figures, by the ten rows in src/data/spikes/breast-her2-positive.ts, and the two one-line
    // state-of-the-art lines by the same file. Every id they referenced is still reachable from the rows that
    // survive; the NCCN guideline summary they cited is now a link on this record. Removed by the September 2026
    // breast family review (docs/BREAST-QA.md), following the lung review's rule 24.
    history: [
      // The five milestones that stood here (1987 Slamon, 1998 trastuzumab, 2013 T-DM1, 2021 DESTINY-Breast03,
      // 2026 T-DXd in early disease) are each written again, with their notes and trials, by
      // src/data/spikes/breast-her2-positive.ts; 1987 rendered as two identical rows under Milestones.
      // `her2` stays reachable from this record's targets.
    ],
    ...
    // The two one-line open problems that stood here (brain metastases; which patients can skip chemotherapy) are
    // written with their trials by src/data/spikes/breast-her2-positive.ts, which is where they now appear once.
