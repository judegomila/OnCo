...
export type Intent = "define" | "treatments" | "biomarkers" | "approval" | "mechanism" | "side-effects" | "trials" | "results" | "compare" | "prognosis" | "who" | "cost" | "general";

export const INTENT_LABEL: Record<Intent, string> = {
  define: "what it means", treatments: "how it is treated", biomarkers: "which biomarkers matter", approval: "approval status", mechanism: "how it works",
  "side-effects": "side effects", trials: "trials", results: "trial results", compare: "comparison", prognosis: "outlook", who: "who and where", cost: "cost and coverage", general: "general",
};
...
const RULES: Rule[] = [
  ...
  { intent: "define", re: [/\bhow (?:common|many cases|big is the market)\b/, /\bincidence\b/, /\bprevalence\b/, /\bmarket size\b/, /\bcases (?:per|a|each) year\b/, /\bdos(?:e|ing|age)\b/, /^what (?:is|are|does|do|was|were|'s)\b/, /\bwhat does .+ mean\b/, /\bmeaning of\b/, /\bdefin(?:e|ition|ed)\b/, /\bexplain\b/, /^what'?s\b/, /^tell me about\b/, /^describe\b/, /\bmeans?\b/, /\bin (?:plain|simple) (?:words|english|terms)\b/, /^(?:so )?what (?:exactly )?(?:is|are)\b/, /\bwhat (?:kind|sort|type) of\b/, /\bis .+ a (?:kind|type|form) of\b/] },
];

/** Classify the question. `entityCount` lets "X vs Y" fall through when only one thing was named; `sameKind` says the two are comparable. */
export function classifyIntent(question: string, entityCount = 2, sameKind = true): Intent {
...
export function kindPriority(intent: Intent, kind: Kind): number {
  const table: Partial<Record<Intent, Partial<Record<Kind, number>>>> = {
    define: { term: 0.5, cancer: 0.45, target: 0.4, technology: 0.4, pathway: 0.38, drug: 0.3, company: 0.3, institution: 0.3, person: 0.3, trial: 0.25, collection: 0.2, bottleneck: 0.25, roadmap: 0.2, paper: 0.2 },
    ...
    who: { company: 0.5, institution: 0.5, person: 0.5, collection: 0.45, drug: 0.4, technology: 0.3, trial: 0.3, cancer: 0.2, term: 0.1 },
    cost: { drug: 0.5, cancer: 0.2, technology: 0.2 },
    general: { cancer: 0.3, drug: 0.3, target: 0.3, technology: 0.3, trial: 0.3, term: 0.25, pathway: 0.25, company: 0.2, institution: 0.2, person: 0.2, collection: 0.2, bottleneck: 0.2 },
  };
...
export function resolveEntities(question: string, index: AskIndex, intent: Intent): { entities: Resolved[]; alternates: AskIndexEntry[] } {
  ...
  for (const s of ordered) {
    ...
    const scored = cands.map((entry) => {
      const strong = isStrong(entry, s.pattern, slice);
      const len = Math.min(s.end - s.start, 24) / 48;
      const score = (strong ? 1 : 0) + kindPriority(intent, entry.kind) + len + 0.1 * (1 - s.start / Math.max(1, text.length)) + (inFocus ? 0.5 : 0) - (generic ? 0.3 : 0);
...
export function analyseQuestion(question: string, index: AskIndex): Analysis {
  ...
  const strong = entities.filter((e) => e.strong);
  const sameKind = strong.length >= 2 && strong[0].entry.kind === strong[1].entry.kind;
  const intent = classifyIntent(question, strong.length, sameKind);
