 ...
 */
const EXPLAINED_PLACEHOLDER = /antibody-drug|adc|conjugate|bispecific|engager|antibody|mab\b|checkpoint|car-t|car t|tcr|til\b|cell therapy|cell-therapy|nk cell|lymphocyte|vaccine|mrna|oncolytic|virus|gene therapy|radioligand|radiopharm|isotope|lutetium|actinium|radium|iodine|test|assay|diagnostic|sequencing|panel|device|software|imaging|cytokine|fusion|enzyme|protein|peptide|hormone analogue/i;
...
export const METRIC_DEFS: MetricDef[] = [
  ...
  },
  {
    ...
    plain: "An approved product should say where it is approved (US, EU, UK, Japan, China, Australia), not just that it is.",
    action: "Add a row for this product in regional-approvals.ts with a regulator source per region.",
    check: (g) => fails(g.kind("drug").filter((d) => APPROVED.has(d.status ?? "")), (d) => (regionalApprovals[d.id] ? null : "no regional row")),
  },
  {
